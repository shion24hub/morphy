...
def remove(
    ...
    y_or_n = typer.prompt(f"Do you really want to remove {fbegin.strftime('%Y%m%d')} - {fend.strftime('%Y%m%d')} data for {symbol}? (y/n)")
    if y_or_n.lower() != 'y':
        print('Canceled.')
        return
    ...
    for date in date_range:
        target = os.path.join(config.STORAGE_DIR, exchange, symbol, f"{date.strftime('%Y%m%d')}.csv.gz")
